...
Added By James Alexander Shield

* Add This Integration[optional] (require registration)

== Dependencies

  $ gem install sinatra dm-core dm-tags dm-timestamps syntax maruku

== Setup

Rename dist.settings.rb to blog.settings.rb and change the Blog config struct 
to your liking.  For security purposes, change the admin password and the 
admin cookie key and value.  These last two can be set to any random value you like, 
just choose something other than the default.
...
Or, modify the DataMapper.setup statement at the top of main.rb.
...
Originally Written by Adam Wiggins

Patches contributed by: Christopher Swenson, S. Brent Faulkner, and Stephen Eley

ORM and other changes by James Alexander Shield
